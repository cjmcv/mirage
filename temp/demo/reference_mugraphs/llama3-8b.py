...
def get_rms_linear():
    ...
    tb_graph = mi.new_threadblock_graph(grid_dim=(192,1,1), block_dim=(128,1,1), thread_num=32, reduction_dimx=64)
...
def get_rms_linear2():
    ...
    tb_graph = mi.new_threadblock_graph(grid_dim=(448,1,1), block_dim=(128,1,1), thread_num=32, reduction_dimx=64)
    tX = tb_graph.new_input(dtensor=X, input_map=(-1, -1, -1), forloop_dim=1)
    tW = tb_graph.new_input(dtensor=W, input_map=(1, -1, -1), forloop_dim=0)
    tM = tb_graph.matmul(tX, tW)
    tAccX = tb_graph.forloop_accum(tX, "rms")
